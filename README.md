...
## 일정 후보 합산 규칙
- 밖에서(/outdoor)와 집에서(/home)의 선택은 같은 사용자별 일정 후보에 합산됩니다.
- 최대 10개까지 유지합니다.
- API POST 응답의 items/count를 화면 상태의 원본으로 사용하여 후보 개수 불일치를 방지합니다.


## 직접 선택한 집에서 활동(OTT) 오늘 일정 반영 수정
- /outdoor, /home에서 사용자가 직접 선택한 후보는 자동 추천 후보보다 우선 배치됩니다.
- A/B/C 스타일 점수가 직접 선택 여부를 뒤집지 않도록 정렬 우선순위를 수정했습니다.
- 선택한 OTT 영화도 시간이 허용되는 한 오늘 일정에 우선 포함됩니다.
- 선택 후보 전체가 남은 시간/예산보다 큰 경우에는 물리적으로 모두 포함되지 않을 수 있습니다.


## 핵심 일정 흐름 단순화
- 사용자가 /outdoor, /home에서 직접 선택한 후보를 오늘 일정의 뼈대로 먼저 배치합니다.
- A/B/C 스타일은 직접 선택 후보를 유지한 상태에서 남는 시간을 자동 추천으로 채웁니다.
- 직접 선택 후보는 스타일별 이동거리 한도 때문에 자동으로 제거하지 않습니다.
- 직접 선택 후보가 빠지는 경우는 고정시간 충돌, 남은 시간 부족, 예산 초과처럼 실제 배치가 불가능한 경우로 제한합니다.
- 선택 후보 반영 수를 일정 카드에서 명확히 표시합니다.
