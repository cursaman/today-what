...
export function scoreActivity(activity: Activity, condition: RecommendationCondition) {
  ...
  const home=String(activity.metadata?.homeTeam??""),away=String(activity.metadata?.awayTeam??"");
  if(condition.favoriteTeams?.some(team=>home.includes(team)||away.includes(team)||team.includes(home)||team.includes(away))) score+=40;
